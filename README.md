...
- 复核结论在 `llm_cache` 里按判定指纹复用（30 天后清理）：指纹覆盖分析文本哈希（正文/caption 与按钮文本，见「关键约定」）、发送者身份、语言、消息特征、规则命中信号与误伤样例（顺序敏感），同一段正文换身份、换命中组合或换样例会重新复核，身份原文不落库。缓存只替代复核这一步，规则命中与 `decide` 每次都照常执行，因此群与群之间的配置差异不会被缓存抹平。
...
- [ ] （可选）在 BotFather 开启 **Bot-to-Bot Communication Mode**：之后 bot 作为管理员会在群里收到其他 bot 的普通消息并纳入审核；不开则收不到这些消息（不是漏审，是 Telegram 不投递）。开启后官方要求自备防循环，本 bot 的约束见「关键约定」
...
`action` 是档位字符串，取 `warn | delete | mute | ban`（申诉页只会看到非放行处置）。`mute` 的截止时刻在 `actionUntil`，其余档位为 `null`。`appeal` 为 `null` 表示还没有人提交申诉；`state` 取 `open | upheld | overturned`。`sampleText` 是处置时的分析文本摘录（≤280 字符，正文/caption 之外可含按钮文本），没有可摘录文本（放行、无正文且无按钮）时为 `null`。
...
处置通知的投递是两态的：先私聊被处置人（附申诉入口按钮）；私聊不可达时回退群内通知（匿名文案 + 申诉按钮）——不可达包括 Telegram 403（从未与 bot 私聊、被拉黑）与 400 `chat not found`（对方是 bot 或已注销账号）。实际落点记在决策上，供上面的生命周期编辑使用；编辑是 best-effort，引用缺失或编辑失败（消息过旧等）只记日志，不影响提交与结案。
...
**正文默认不留存，处置对象留摘录。** `message_events` 只有 `content_hash` 与特征列（是否有链接、媒体类型、长度、自定义表情数、表情总数、是否经内联机器人发送）；唯一的文本落地形态是 `sample_text`，一条被判非放行的消息的分析文本摘录（≤280 字符；分析文本可能含按钮文本，见下），用于申诉复核与事后复盘。它由 SQL 条件强制：只有同事件存在非 `pass` 决策时才允许写入，放行消息的文本没有任何写入路径。

**分析对象是正文/caption + 内联键盘按钮文本。** 广告号常把联系方式、引流话术放进按钮（正文只留一句人话），只审正文会漏判。bot 层把内联键盘按行展平，按钮 `text` 先 `trim`、跳过空串、最多取前 10 个，单个标签截断到 64 个 Unicode 码点，然后逐行以 `(btn)` 前缀拼在正文/caption 之后（正文为空时不带前导换行、首行即 `(btn)`）；没有按钮或有效标签为空时文本原样不变。标记用 ASCII 括号 + 拉丁字母而不是 `【按钮】` 一类全角段标记：全角标点夹在汉字之间会被 `normalize` 当拆词符整段删除，段标记会被吃掉、相邻正文与标签还会粘连；`(btn)` 不属于折叠条件，经归一化原样存活（分隔换行折叠为空格），规则匹配与复核都能看见按钮来源。组合后的文本是唯一的审核输入：归一化、规则匹配、复核提示词、内容哈希、owner 判定 feed 与 `sample_text` 摘录都读它（新消息与编辑消息同一路径）。回复键盘、强制回复等非内联形态是客户端 UI、不携带可审文本，不参与组合。

**按钮标记切换（一次性迁移后果）。** 本批把按钮段标记从 `【按钮】` 改为逐行 `(btn)` 前缀，标记参与分析文本，因此内容哈希口径随之变化，部署切换时有三项一次性后果：`llm_cache` 的既有条目全部不再命中，灰色地带复核会重跑一遍（旧行由 30 天保留期清理）；部署前入库的撤销样本 `sample_text` 带旧标记，与新消息哈希永久失配，这批样本的内容白名单不再命中（样本回看窗口 90 天，自然过期）；切换窗口内对同一条消息的编辑，判别符 `edit:${edit_date}:${内容哈希前 16 位}` 会变化，被当作一次新的编辑重新审核。

**其他 bot 的消息一并审核（Bot-to-Bot Communication Mode）。** 在 @BotFather 为本 bot 开启该模式且它是群管理员时，Telegram 会把群里其他 bot 的普通消息投递给它；入口过滤只跳过本 bot 自身（避免自触发），其他 bot 的消息按普通成员语义走同一条管线——判定、摘录、处置与申诉流程和人类消息一致，信任名单同样可以豁免。对 bot 的处置也按普通成员语义执行：删除消息最可靠；对 bot 禁言/封禁没有专用语义，Telegram 拒绝时走既有终态处理（决策终结、私聊 owner 告知，不无限重试），网络类失败不属终态、由补偿扫描在 24 小时窗口内继续重试。官方要求开启该模式的部署自备防循环：本 bot 只做单向审核，不与其他 bot 对话；对 bot 目标的处置通知会先尝试私聊（bot 之间通常不可达，Telegram 返回 400 `chat not found`），失败即回退群内通知。回退的群内通知是一条普通群消息，出现在群里后可能被其他 bot（包括被审核的对方 bot）读到并触发其处理，形成跨 bot 的间接输入——本服务无法控制他方 bot，这条残余风险记录在此。事件与决策 id 由 `(chatId, messageId)` 确定性派生，执行侧另有 `eventId + action` 幂等闸门与 `executed` 回填，重投递不会再生动作，本 bot 自身消息也在入口被跳过，因此不存在「处置动作 → 对方新消息 → 再被审核」的循环。某个友方 bot 的正常消息被误伤时，把它临时加入该群的信任名单即可豁免。
