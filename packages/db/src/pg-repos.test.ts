...
describe('写入语句的形状与参数化', () => {
  ...
  test('结案只对未结案记录生效，条件更新返回是否真的结了案', async () => {
    ...
    const claimedResult = await claimed.repos.appeals.resolve(appealId, 'overturned', resolvedAt, asUserId(1_000_000_001))

    expect(claimedResult).toBe(true)
    const [statement] = claimed.recorded
    expect(statement?.query).toContain('set "state" = $1, "resolved_at" = $2, "resolved_by" = $3')
    expect(statement?.query).toContain('"state" = ')
    expect(statement?.query).toContain('returning')
    expect(statement?.params).toEqual(['overturned', '2026-09-23T10:30:00.000Z', 1_000_000_001, appealId, 'open'])

    // 0 行（并发重复点击）时返回 false：调用方据此不再回滚权限、也不再报成功。
    const missed = createPgReposRecording([])
    expect(await missed.repos.appeals.resolve(appealId, 'overturned', resolvedAt, asUserId(1_000_000_001))).toBe(false)
  })
